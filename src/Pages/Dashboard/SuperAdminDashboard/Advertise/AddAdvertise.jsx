...
export default function AddAdvertise() {
  ...
  const onSubmit = (data) => {
    ...
    if(imageUrls.length === 0){
      ...
      axiosSecure.post('/advertise', Data)
        .then(res => {
          console.log(res)
          if (res.data.insertedId) {
            toast.success(
              "Successfully added the advertise",
            );
            refetch()
            reset({
              title: "",
  ...
  return (
    <div className="py-10">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 p-4 w-[90%] m-auto">
        <h1 className="text-3xl font-semibold">Add Advertisment</h1>
        ...
        <input required {...register("title")} placeholder="Blog Title..." className="border rounded-lg  p-2 w-full" />
